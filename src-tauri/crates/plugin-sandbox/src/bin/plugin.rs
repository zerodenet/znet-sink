...
fn run() -> Result<()> {
    ...
    match args.as_slice() {
        ...
        [cmd, root, seed, output, metadata] if cmd == "pack" => pack_application(root, seed, output, metadata, None)?,
        [cmd, root, seed, output, metadata, registration] if cmd == "pack" => pack_application(root, seed, output, metadata, Some(registration))?,
        [cmd, manifest, source, seed, output, metadata] if cmd == "pack-legacy" => pack_legacy(manifest, source, seed, output, metadata, None)?,
        [cmd, manifest, source, seed, output, metadata, registration] if cmd == "pack-legacy" => pack_legacy(manifest, source, seed, output, metadata, Some(registration))?,
        _ => return Err("usage: znet-plugin catalog | releases ID | list ROOT | install ROOT ID TAG | inspect ROOT ID | rollback ROOT ID | remove ROOT ID | pack APP_ROOT SEED_FILE PACKAGE_OUT METADATA_OUT [REGISTRATION_JSON] | pack-legacy MANIFEST SOURCE SEED_FILE PACKAGE_OUT METADATA_OUT [REGISTRATION_JSON]".into()),
    }
    Ok(())
}
...
fn pack_application(
    ...
    if !manifest.files.is_empty() {
        return Err("plugin.json files index is generated by pack and must be empty".into());
    }

...
pub(crate) async fn restart_intent<Q, F>(
    saved: Option<bool>,
    running: bool,
    query: Q,
) -> AppResult<bool>
...
pub(crate) async fn restore<Q, QF, E, EF>(
    query: Q,
    enable: E,
    timeout: Duration,
    interval: Duration,
...
{
    tokio::time::timeout(timeout, restore_inner(query, enable, timeout, interval))
        .await
        .unwrap_or_else(|_| {
            Err(AppError {
                code: "tun_restore_unconfirmed",
                message: "TUN restoration deadline elapsed; runtime outcome remains unconfirmed"
                    .into(),
                details: None,
            })
        })
...
async fn restore_inner<Q, QF, E, EF>(
    mut query: Q,
    mut enable: E,
    timeout: Duration,
    interval: Duration,

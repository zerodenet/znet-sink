...
fn restore_idle_proxy(
    ...
    if matches!(
        state.runtime_host().process_status()?.state,
        CoreProcessState::Starting | CoreProcessState::Running
    ) {
        return Ok(false);
    }
...
fn cleanup_failed_connect(state: State<'_, AppState>) {
    if let Err(error) = system_proxy_guard::disable_with_guard() {
        let message = format!("failed connection proxy cleanup: {}", error.message);
        state.runtime_host().record_error(message.clone());
        crate::services::file_logger::line(&message);
    }
}

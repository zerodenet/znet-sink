> 2026-09-12 更新：桌面客户端已接入“设置 → 插件”，可从本地选择 `.zspkg`，按固定中央目录登记和包签名校验后安装，并逐项授权、运行、停用和卸载。安装数据复用 Store，位于客户端数据目录的 `plugins/`；当前仅开放自身信息，GUI Release 发现/回滚及业务 SDK 尚未完成。完整进度见 [迁移记录](./capability-migration.md)。下文较早阶段的 GUI 未接入状态保留作实施历史。

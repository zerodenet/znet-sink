...
2026-09-12：插件与受保护配置的后续实施以 [客户端统一入口、管理与执行管道](./unified-capability-plan.md) 为准，所有内置与外部调用进入同一客户端管理及执行路径，按能力迁移并接入 SDK，避免双执行链。
...
2026-09-11 补充：已单独启动 [插件沙箱与能力授权基础阶段](./plugin-foundation.md)。下文各历史批次的“不含插件”范围仍描述当时交付；当前新增仅为本地实验运行器，不代表生产加载器、移动端或受保护配置已完成。

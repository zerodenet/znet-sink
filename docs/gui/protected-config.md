...
状态：受保护业务链尚未实现。2026-09-12 已按用户场景确定 [ZBoard 联动插件主线目标](./zboard-plugin-target.md)，覆盖源、账号/第三方登录、公告通知、受保护订阅、本地密文缓存和内核内存运行。该目标优先于下文历史候选：当前采用客户端统一管理的组件 VM，独立 worker/OS 沙箱不再是前置条件。下文 2026-09-08 的源码状态与接口清单是历史盘点，实施前需重新核对。

...
每次替换内核前，在应用数据目录的 `kernel-rollback/upgrade-*` 中保留即将改写的二进制、伴随文件、安装清单，以及 `previous-app-config.json`。`receipt.json` 记录目标目录和 `pending`、`committed`、`rolled_back` 状态。文件通过同目录临时副本原子替换，运行中的旧内核在下载和预检查期间继续工作。

安装或恢复运行状态失败时自动尝试回退。回退未完成时错误中返回备份路径和逐项失败原因；成功安装也保留备份。目前没有备份自动清理和强制终止升级后的启动时自动续回退，`pending` 记录需人工检查后恢复，不能视为已完成安装。

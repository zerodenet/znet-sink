...
assert.match(service, /if \(draft\.mode === 'disabled'\) delete runtime\.dns/);
assert.match(service, /runtime\.tun\.dns_hijack = draft\.mode !== 'disabled' && draft\.dnsHijack/);
assert.match(service, /rule\.server === oldName \? name : rule\.server/);
assert.match(service, /guiValidateDnsConfig\(next\)/);
assert.match(service, /await guiApplyDnsConfig\(source\)\.catch/);
assert.match(service, /proxyConfigSignal\.markChanged\(true\)/);

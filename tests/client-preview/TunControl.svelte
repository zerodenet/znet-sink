<script lang="ts">
  ...
  import { guiState, coreEvents } from './state.svelte';
  import OperationFeedback from './OperationFeedback.svelte';

  let { model, onInspect }: { model: OverviewModel; onInspect: () => void } = $props();
  const tun = $derived(guiState.tunStatus);
  const failed = $derived(model.ready && tun.enabled && !tun.healthy);
  const healthy = $derived(model.ready && tun.enabled && tun.healthy);
  const egressReady = $derived(model.ready && tun.enabled && tun.ipv4Egress.availability === 'available' && (!tun.dualStack || tun.ipv6Egress.availability === 'available'));
</script>

<!-- A compact arrangement of the client's existing feature card and Switch. -->
<section class="feature-card" aria-label="TUN 与网络栈">
  ...
  <div class="feature-row">
    ...
    <div class="feature-copy"><div class="feature-main"><span class="feature-name">TUN 网卡</span><strong class:danger={failed}>{guiState.isSwitchingTun ? '切换中…' : model.tunLabel}</strong></div><span class="feature-meta">{model.ready && tun.enabled ? `${tun.name} · ${tun.autoRoute ? '自动路由' : '手动路由'}` : '接管系统网络流量'}</span></div>
    <Switch bind:checked={() => guiState.isTunSwitchOn, () => void guiState.toggleTun()} disabled={guiState.isTunSwitchOn ? !guiState.canDisableTun : !guiState.canEnableTun} aria-label={guiState.isTunSwitchOn ? '关闭 TUN 并取消自动恢复' : '开启 TUN'} />
  </div>
  <div class="feature-row"><span class="feature-dot" class:healthy={model.ready && coreEvents.stackState === 'started'}></span><span class="feature-name">内核网络栈</span><strong>{model.ready ? coreEvents.stackMode : '待确认'}</strong></div>
  <div class="egress-summary" class:danger={failed}>{!model.ready ? '等待内核确认接管状态' : !tun.enabled ? 'TUN 未接管流量' : failed ? '出口异常 · 下方可查看原因' : egressReady ? `${tun.dualStack ? 'IPv4 / IPv6' : 'IPv4'} 出口可用` : '出口状态待确认'}</div>

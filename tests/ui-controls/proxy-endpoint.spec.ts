...
test('client endpoint remains editable when a legacy profile source marker exists', async ({ page }) => {
  ...
  await expect(page.getByText('修改后只对当前配置生效', {exact:false})).toBeVisible();
});
...
test('managed endpoint rejects partial numbers and saves an exact numeric port', async ({ page }) => {
  ...
  const save = page.getByRole('button', { name: '应用', exact: true });
  ...
  await save.click();
  await expect(page.getByLabel('保存结果')).toContainText('"localProxy.port":9000');
